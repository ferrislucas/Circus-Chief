<template>
  <form v-if="canSendMessage || isScheduledForFuture" @submit.prevent="(isDraft || isScheduledDraft) ? $emit('start') : $emit('send')" class="input-form">
    <ResizableTextarea
      ref="textareaRef"
      v-model="modelInput"
    ...
    <div v-if="!isScheduledForFuture" class="input-controls">
      <div class="session-options">
        ...
        <ModelSelector v-model="modelSelectedModel" />

        <FileAttachment ref="fileAttachmentRef" @update:files="$emit('update:attachedFiles', $event)" />
        <SlashCommandButton
          v-if="workingDirectory"
          @open="$emit('openSlashCommandWizard')"
        />
...
const fileAttachmentRef = ref(null);
...
// Two-way binding for selected model
const modelSelectedModel = computed({
