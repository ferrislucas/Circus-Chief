<template>
  ...
  >
    ...
    <div v-if="isOpen" class="dialog-backdrop" @click.self="close">
      <section ref="dialogRef" class="allowance-dialog" role="dialog" aria-modal="true" aria-labelledby="provider-allowance-title" tabindex="-1" @keydown.esc="close">
...
function ariaLabel(snapshot) {
  ...
  return `${snapshot.providerName}: ${statusText(snapshot.status)}, ${value}`;
}
