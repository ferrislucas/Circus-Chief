 ...
 *
 * When an active member exists, every reference becomes that concrete member
 * (and the paired provider field is persisted where the schema has one).
 * When none exists, the selection is cleared, which is the product's existing
 * "use the configured/default model" behavior for each surface.
 *
 * @param {string} tierId
