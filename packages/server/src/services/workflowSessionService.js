...
function clearExecutableMemberState(db, runId, reason, time) {
  return db.prepare(`UPDATE sessions SET own_work_state='cancelled', own_work_closed_at=?, workflow_reason=?,
    ...
    scheduled_at=NULL, pending_prompt=NULL, pending_model=NULL, pending_provider_id=NULL, pending_conversation_id=NULL,
...
export function claimWorkflowSessionStart(sessionId) {
  return databaseManager.transaction(() => {
    ...
    const time = now();
    db.prepare(`UPDATE sessions SET scheduled_at=NULL, pending_prompt=NULL, pending_model=NULL, pending_provider_id=NULL,
