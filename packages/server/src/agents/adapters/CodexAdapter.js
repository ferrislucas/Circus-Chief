...
export class CodexAdapter extends BaseAgent {
  ...
  async *_executeCli(queryParams, options) {
    ...
    const rolloutWatcher = this._maybeCreateRolloutWatcher(options);
    if (!rolloutWatcher) {
  ...
  /**
   * ChatGPT-plan sessions report rate limits only through their rollout
   * files, so a qualifying CLI execution tails the active rollout for
   * `token_count` events (plan §7.1). Skipped when the global app-server
   * meter is healthy — it is the single writer for these providers (§7.2).
   */
  _maybeCreateRolloutWatcher(options) {
    ...
    return createCodexRolloutWatcher({
      providerId: options.providerId,
      allowanceObserver: this._allowance.allowanceObserver,
      env: options.env,
      clock: this._allowance.clock,
      streamStaleMsProvider: getStreamStaleAfterMs,
    });
  }
