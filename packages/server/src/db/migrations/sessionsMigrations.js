...
const SESSIONS_ALL_CURRENT_COLUMNS = `
    id TEXT PRIMARY KEY,
    project_id TEXT NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    status TEXT NOT NULL DEFAULT 'starting' CHECK (status IN ('starting', 'running', 'waiting', 'stopped', 'completed', 'error', 'scheduled')),
...
const SESSIONS_ALL_CURRENT_COLUMN_NAMES = [
  'id', 'project_id', 'name', 'status', 'mode', 'thinking_enabled',
...
function recreateSessionsTable(db, columnsSql, allColumnNames) {
...
function migrateSessionsDefaultModeAndThinking(db) {
