...
export class SessionRepository extends BaseRepository {
  ...
  getWorkspaceCards(projectId, { archived = false, starred = null, status = null, scheduled = null, limit = 50, offset = 0 } = {}) {
    ...
    const sql = `
      ...
      FROM sessions s
      JOIN aggregates a ON a.root_id = s.id
      ...
      LIMIT ? OFFSET ?`;
    // tree's project parameter is intentionally separate from the outer query.
    const rows = this.db.prepare(sql).all(projectId, ...params, limit, offset);
    return rows.map(row => ({
