...
describe('sessionManager model switching mid-conversation', () => {
  ...
  describe('continueSession with model parameter', () => {
    it('passes model to SDK when provided', async () => {
      ...
      await continueSession(session.id, 'follow-up message', '/tmp/test', { model: 'claude-sonnet-5' });
    ...
    it('resumes session when model has not changed', async () => {
      ...
      await runSession(session.id, 'initial prompt', '/tmp/test', { model: 'claude-opus-4-6' });
      ...
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-opus-4-6'));
      await continueSession(session.id, 'follow-up message', '/tmp/test', { model: 'claude-opus-4-6' });
    ...
    it('does NOT resume session when model changes', async () => {
      ...
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-sonnet-5'));
      await continueSession(session.id, 'follow-up message', '/tmp/test', { model: 'claude-sonnet-5' });
  ...
  describe('conversation context format', () => {
    it('includes previous messages in conversation context when switching models', async () => {
      ...
      // First turn
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-opus-4-6'));
      await runSession(session.id, 'Hello, can you help me?', '/tmp/test', { model: 'claude-opus-4-6' });
      ...
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-sonnet-5'));
      await continueSession(session.id, 'Now use sonnet please', '/tmp/test', { model: 'claude-sonnet-5' });
    ...
    it('does not include context when there are no previous messages', async () => {
      ...
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-opus-4-6'));
      await runSession(session.id, 'First message', '/tmp/test', { model: 'claude-opus-4-6' });
  ...
  describe('model change detection via session.model (not conversation.model)', () => {
    it('continueSession detects model change via session.model and includes conversation context', async () => {
      ...
      await continueSession(session.id, 'follow-up message', '/tmp/test', { model: 'claude-sonnet-4-20250514' });
    ...
    it('continueSession with model: null does not trigger modelChanged and does not overwrite session.model', async () => {
      ...
      await continueSession(session.id, 'follow-up with no model', '/tmp/test');
    ...
    it('continueSessionWithExistingMessage detects model change via session.model and includes context', async () => {
      ...
      session = sessions.create(project.id, 'Test Session', 'initial prompt', 'standard');
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-opus-4-20250514'));
      await runSession(session.id, 'initial prompt', '/tmp/test', { model: 'claude-opus-4-20250514' });
  ...
  describe('model switching between providers', () => {
    it('allows switching from one model to another across messages', async () => {
      ...
      // First with opus
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-opus-4-6'));
      await runSession(session.id, 'start with opus', '/tmp/test', { model: 'claude-opus-4-6' });
      ...
      // Then sonnet
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-sonnet-5'));
      await continueSession(session.id, 'now use sonnet', '/tmp/test', { model: 'claude-sonnet-5' });

      // Check models were passed correctly
      expect(mockQuery.mock.calls[0][0].options.model).toBe('claude-opus-4-6');
      expect(mockQuery.mock.calls[1][0].options.model).toBe('claude-sonnet-5');
    });
    ...
    it('tracks model changes across multiple switches', async () => {
      ...
      // Opus
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-opus-4-6'));
      await runSession(session.id, 'opus turn', '/tmp/test', { model: 'claude-opus-4-6' });
      ...
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-sonnet-5'));
      await continueSession(session.id, 'sonnet turn', '/tmp/test', { model: 'claude-sonnet-5' });

      conv = conversations.getActiveBySessionId(session.id);
      conversations.update(conv.id, { model: 'claude-sonnet-5' });

      // Haiku - should not resume
      mockQuery.mockImplementationOnce(() => createMockQueryResponse('claude-3-5-haiku-latest'));
      await continueSession(session.id, 'haiku turn', '/tmp/test', { model: 'claude-3-5-haiku-latest' });
