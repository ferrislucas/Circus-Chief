...
- The `<<autonomous-loop-dynamic>>` / `<<autonomous-loop>>` prompt sentinels have no meaning outside the SDK's loop state. Rather than substitute a generic `Continue` for a request we can't actually honor, the wakeup is **refused**: nothing is scheduled, and the transcript gets a `tool_output` work log explaining why, pointing the agent at `POST /api/sessions/:id/schedule` with an explicit prompt instead.
