...
test.describe('Template Configuration Fields', () => {
  ...
  test('saves and returns model field correctly', async () => {
    ...
    const updated = await updateTemplate(template.id, { model: 'claude-sonnet-5' });
    expect(updated.model).toBe('claude-sonnet-5');

    const fetched = await getTemplate(template.id);
    expect(fetched.model).toBe('claude-sonnet-5');
  });
  ...
  test('template edit form shows saved field values', async ({ page }) => {
    ...
    // Verify metadata badges are displayed on the template card. Scope to this
    // template's card to avoid strict-mode violations when other templates
    // (project or global) exist in the database.
...
test.describe('Auto-Trigger Mechanism', () => {
  ...
  test('child session inherits template settings (mode, model, thinking)', async () => {
    ...
    expect(childSession.model).toBe('claude-sonnet-5');
  });
